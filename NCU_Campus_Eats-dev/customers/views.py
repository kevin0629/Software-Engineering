...
def fetch_cart_item(customer_id): # 抓取目前所擁有的所有購物車
    with get_session() as session:
        query = text("""
            SELECT order_detail.order_detail_id, order_table.order_id, order_detail.item_id, menu_item.item_name, menu_item.price, order_detail.quantity, menu_item.restaurant_id, restaurant.restaurant_name
            FROM order_table
        ...
        for row in result:
            ...
            item_total_price = item_price * item_quantity # 計算單筆價錢
            ...
            grouped_cart_items[restaurant_id]["items"].append({
                ...
                "item_total_price": item_total_price  # 為每個商品計算小計
            })
...
def checkout_order(): # 送出訂單
    if request.method == 'POST':
        ...
        with get_session() as db_session:
            ...
            if order:
                order.order_status = 1  # 設定狀態為 2 表示已送出
                order.total_amount = total_price  # 更新總金額

                db_session.commit()  # 提交變更
            
        return redirect(url_for('customers.view_cart'))  # 重定向到購物車頁面或者其他頁面
